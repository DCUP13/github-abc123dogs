...
export default function MemberDetailDialog({ memberId, memberName, memberEmail, onClose }: MemberDetailDialogProps) {
    ...
    try {
      ...
      const [sesEmailsRes, googleEmailsRes, domainsRes, settingsRes, statsRes, orgDomainsRes] = await Promise.all([
        ...
        supabase
          ...
          .maybeSingle(),
        orgId
          ? supabase.from('organization_domains').select('id, domain').eq('organization_id', orgId).order('domain', { ascending: true })
  ...
  // Compute available domains (org domains not yet assigned to this member)
  const assignedDomainNames = new Set(domains.map(d => d.domain));
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-5xl w-full mx-4 my-8 max-h-[90vh] overflow-y-auto">
        ...
        {activeTab === 'domains' && (
          <div className="space-y-6">
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">Assign Domain</h3>
              {orgDomains.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  No organization domains available. Add domains via Amazon SES settings or Organization Settings first.
                </p>
              ) : availableDomains.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  All organization domains are already assigned to this member.
                </p>
              ) : (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Assigned Domains</h3>
              {domains.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 dark:bg-gray-900 rounded-lg">
                  <Globe className="w-12 h-12 text-gray-400 mx-auto mb-2" />
                  <p className="text-gray-500 dark:text-gray-400">No domains assigned</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {domains.map((domain) => (
