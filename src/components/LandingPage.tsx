...
export function LandingPage({ onSignInClick, onCreateAccountClick, onPrivacyClick, onTermsClick, onCookieClick, onADAClick }: LandingPageProps) {
  ...
  const onQuizClick = () => { window.dispatchEvent(new CustomEvent('navigate-to-quiz')); setMobileMenuOpen(false); };
      ...
      <header className="fixed top-0 left-0 right-0 bg-om-forest-deep/95 backdrop-blur-md border-b border-om-forest z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-base md:text-lg text-om-tan">
            ...
            <button onClick={onQuizClick} className="hover:text-om-parchment transition-colors text-om-gold">Quiz</button>
        ...
        {mobileMenuOpen && (
          <div id="mobile-menu" className="md:hidden bg-om-forest-deep border-t border-om-forest px-6 py-4 flex flex-col gap-4">
            ...
            <button onClick={onQuizClick} className="text-left text-base text-om-gold hover:text-om-parchment transition-colors py-1">Quiz</button>
